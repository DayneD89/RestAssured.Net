    ...
    public class ResponseContext
    {
        ...
        private void ParseLoad()
        {
            if (_loadResponses.Any())
            {
                ...
                {
                    _loadValues.Add(LoadValueTypes.AverageTTLMs.Value, new TimeSpan((long)_loadResponses.Where(x => (x.StatusCode/100) == 2).Average(x => x.Ticks)).TotalMilliseconds);
                    _loadValues.Add(LoadValueTypes.MaximumTTLMs.Value, new TimeSpan(_loadResponses.Where(x => (x.StatusCode / 100) == 2).Max(x => x.Ticks)).TotalMilliseconds);
                    _loadValues.Add(LoadValueTypes.MinimumTTLMs.Value, new TimeSpan(_loadResponses.Where(x => (x.StatusCode / 100) == 2).Min(x => x.Ticks)).TotalMilliseconds);
                }
                else
                {
